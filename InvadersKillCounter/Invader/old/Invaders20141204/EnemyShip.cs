...
namespace Scripts.KSWH
{
    class EnemyShip
    {
        ...
        public void Update(int count)
        {
            ...
            {
                //目標地点周辺に何かある場合、再度補正
                var sphere = new BoundingSphere(dest, sizeFactor);
            ...
            Vector3 dir = Target.GetPosition() - myShip.GetPosition();
            ...
            //どう働く？
            //*10fの目的？回転の強さ？
            var rot = new Vector2((float)x, (float)y) * 10f;
        ...
        private void UpdateTarget()
        {
            ...
            //search next target
            var bs = new BoundingSphere(myShip.GetPosition(), Range);
            ...
            foreach (var entity in ents)
            {
                ...
                if (nextShip != null)
                {
                        ...
                        (nextShip.DisplayName != null && !nextShip.DisplayName.Contains(Invaders.Identifier)))
                    {
                        ...
                        // heavy proc?
                        //if (Target == null
                        //    || ((Target.GetPosition() - myShip.GetPosition()).Length()) > (nextShip.GetPosition() - myShip.GetPosition()).Length())
                        //{
                        //    Target = entity;
                        //}
                    }
        ...
        {
            var lst = new List<IMySlimBlock>();
            myShip.GetBlocks(lst, (x) => x.FatBlock != null && x.FatBlock.BlockDefinition.SubtypeId.Contains("Gyro"));
            if (lst == null || lst.Count == 0)
                return;
            var gyro = lst[0];
